    ...
    <div class="col bg-light-subtle rounded border">
      <div class="row">
            ...
            <div class="col-auto text-center">
              ...
              @else
                @if ($irs[$i-1]->validasi == 0)
                  Validasi : <span id="info_validasi" class="text-danger">belum</span>
                @else
                  Validasi : <span id="info_validasi" class="text-success">sudah</span>
                @endif
              @endif    
        ...
        <div class="col-auto my-auto ms-auto">
          @if ($irs[$i-1] != null)
            @if ($irs[$i-1]->validasi == 0)
              <button type="button" id="validasi" class="btn btn-success btn-sm" data-nim="{{ $nim }}" data-smt="{{ $i }}">Validasi</button>
            @else
              <button type="button" id="validasi" class="btn btn-danger btn-sm" data-nim="{{ $nim }}" data-smt="{{ $i }}">Batalkan Validasi</button>
            @endif
          @endif
        </div>
